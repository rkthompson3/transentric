define(['text!templates/common/commonTemplate.html!strip','text!templates/events/nextEvent/nexteventViewTemplate.html!strip','model/user/userModel',"common"],
       function (commonTemplate,nexteventViewTemplate,user,aem) {
  var common = $(commonTemplate).siblings('script');
  var html = $(nexteventViewTemplate).siblings('script');
  var UrlString=aem.commonUrl();
  var orderPageView = Backbone.View.extend({
	  header:_.template(common.siblings("#all-header").text()),
	  content:_.template(html.siblings("#nextevent").text()),
	  ...
	  render: function(){
		  ...
		  $(this.el).find("#headertext").html('Next Event');
		  var _this = this;
		  _(function() {
	      ...
	      events:{
	    	  'click #viewSelectEvent':'viewSelectEvent_clickHandler',
	    	  'click #requiredEvents':'requiredEvents_clickHandler' ,
	    	  'click #reportdamagedelay':'reportdamagedelay_clickHandler'
	      },
	      viewSelectEvent_clickHandler:function(event){
	    	 router.navigate("optionalselectevent",{trigger:'true'}); 
	      },
	      requiredEvents_clickHandler:function (event){
		  ...
		  reportdamagedelay_clickHandler:function(event){
			  router.navigate("damagedelayevent",{trigger:'true'});
		  },
		    	 ...
		    	 var data= user.get("data");
	  			    ...
	  			    if(data.stops[subdata].docIds !== null){
					    for(var i=0;i<data.stops[subdata].docIds.length;i++){
						   ...
						   $(this.el).find('li#requiredEvents').find('.Tick'+documentcode).css({"display":"block"});
					   }
				    }
	  			    var Raisedeventcode=[];
	  			    if(data.raisedEvents !== null || data.raisedEvents.length > 0){
					    for(var i=0;i<data.raisedEvents.length;i++){
                           ...
                           var CurrentHtmlText = $(this.el).find('li#requiredEvents').find('.Event'+eventcode).text()+"</br>"+eventDate;
                           Raisedeventcode.push(eventcode);
                     	   $(this.el).find('li#requiredEvents').find('.Event'+eventcode).html(CurrentHtmlText);
                     	   $(this.el).find('li#requiredEvents').find('.Tick'+eventcode).css({"display":"block"});
					       }
					   }
				    }
	     			   // get all vin numbers
	     			...
	     			if(allPresentVinNumbers.length != 0 && (allPresentVinNumbers.length == getallRaisedVinNumber.length)){
                        $(this.el).find('li#requiredEvents').find('.TickInspection').css({"display":"block"});
                    }  
	  			...
	  			// condition for if all optional events are disabled and none of them got raised. 
	  			//alert(user.get("requiredCurrentIndex"));  //required event current index.
	  			var unraisedoptseq;
	  			for (var i in workflowarr){
	  				if(workflowarr[i].workflowType.name == "Order in the System"){
	  				$.each(workflowarr[i].details,function(index,val){if(val.required === false){
	  					var str=val.stopType.codes + '';
	  					if(val.eventDetail !== null && val.workflowDetailType == "EVENT" && jQuery.inArray(val.eventDetail.eventCode,Raisedeventcode ) ==-1 && jQuery.inArray(GetstopReason, str.replace(/,\s+/g, ',').split(',')) != -1){
	  						unraisedoptseq=val.sequenceNumber;
	  						return false;
	  					}
	  					else if(val.eventDetail === null && val.workflowDetailType == "DOCUMENT" && jQuery.inArray(val.documentDetail.documentType.id,RaisedDocumentcode ) ==-1  && jQuery.inArray(GetstopReason, str.replace(/,\s+/g, ',').split(',')) != -1){
	  						unraisedoptseq=val.sequenceNumber;
	  			...
	  			// Hiding optional event li from next event template.
	  			if((user.get("requiredCurrentIndex") < unraisedoptseq) && (user.get("requiredCurrentIndex")!=0)){
	  				$(this.el).find('#viewSelectEvent').css('display','none');
	  			}
		      }
